...
var _ = g.Describe("[sig-installer][Suite:openshift/openstack] The Openshift", func() {
	...
	g.Context("on cloud provider configuration", func() {
		...
		g.It("should update use-octavia and enabled properties depending on the NetworkType", func() {
			...
			if network.Status.NetworkType == "Kuryr" {
				//Fixed values regardless the content of userCfg.
				expectedValues = []string{"true", "false"}
			} else {
				//Should map the values configured on the userCfg.
				for _, propertyName := range propertyNames {
					value, err := getPropertyValue(sectionName, propertyName, userCfg)
					o.Expect(err).NotTo(o.HaveOccurred(), "Error getting the property %q in section %q on configMap "+
						"%q (namespace: %q)", propertyName, sectionName, userConfigMap.name, userConfigMap.namespace)
